...
public class Matcher<T> where T : System.Enum {
  ...
  private readonly List<ICmdCommand<T>> _commands;
  ...
  public T Parse(string? text) {
    ...
    List<ICmdCommand<T>> matches = new List<ICmdCommand<T>>();
  ...
  public List<ICmdCommand<T>> Commands {
    init {
      if (value is null) {
        _commands = new List<ICmdCommand<T>>();
      } else {
        _commands = new List<ICmdCommand<T>>(value);
      }
